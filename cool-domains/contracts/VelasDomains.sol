...
contract Domains is ERC721URIStorage {
    ...
    //mapping data to store their names
    ...
    constructor(
        ...
        tld = _tld;
        console.log("%s Your name service deployed successfully", _tld);
    }
    ...
    function price(string calldata name) public pure returns (uint) {
        ...
        require(len > 0);
        if (len == 3) {
            return 0.5 * 10 ** 17; // 5 ETL = 5 000 000 000 000 000 00 (18 decimals). We're going with 0.05 ETL cause the faucets don't give a lot
        } else if (len == 4) {
    ...
    function register(string calldata name) public payable {
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the ronkips name service", "image": "data:image/svg+xml;base64,',
    ...
    // This will give us the owner's domain address
    function getAddress(string calldata name) public view returns (address) {
        //the function returns the domain owner's wallet address as the native Solidity address type.
        return domains[name];
    }
